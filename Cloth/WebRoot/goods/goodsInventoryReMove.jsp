...
<html>
  <head>
    ...
    <title>goodsInventoryReMove.jsp</title>
  ...
  <body>
    <center>
    	<c:if test="${!empty gi}">
    		<form action="goodsInventory/updateOneReMove.action" method="post">
		    	<input type="hidden" name="goodsInventoryId" value="${gi.goodsInventoryId }"/>
  				<center><h2>现有商品出库</h2></center>
		   		<table border="1" cellpadding="0" cellspacing="0" bordercolor="green">
					...
					<tr><th>出货数:</th><td><input name="num" requred/></td></tr>
					<tr><td colspan="2"><input value="出货" type="submit"/>&nbsp;&nbsp;<a href="goodsInventory/selectAllByPager.action">返回</a></td></tr>
		    	</table>
		    </form>
    	</c:if>
    </center>
